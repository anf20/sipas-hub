<div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
    <flux:header>
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="{{ route('finance.hub', ['tab' => 'spp']) }}" wire:navigate>{{ __('Manajemen SPP') }}</flux:breadcrumbs.item>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <div class="flex items-center gap-3">
                    <flux:heading size="xl">Detail SPP {{ $monthName }}</flux:heading>
                    <flux:badge size="sm" inset="top">SPP Bulanan</flux:badge>
                </div>
                <flux:subheading>
                    {{ __('Daftar tagihan SPP untuk bulan ') }} {{ $monthName }}
                </flux:subheading>
            </div>
        </div>
                ...
                <div class="flex flex-col md:flex-row justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <flux:radio.group wire:model.live="status" variant="segmented" size="sm">
                            ...
                            <flux:radio value="inactive" label="{{ __('Inactive') }}" />
                ...
                <flux:table.rows>
                    @foreach($invoices as $invoice)
                        <flux:table.row :key="$invoice->id">
                            <flux:table.cell font-weight="medium">{{ optional($invoice->student)->name ?? '-' }}</flux:table.cell>
                            <flux:table.cell>{{ optional($invoice->student)->nis ?? '-' }}</flux:table.cell>
                            <flux:table.cell>{{ optional(optional($invoice->student)->schoolClass)->name ?? '-' }}</flux:table.cell>
                            <flux:table.cell>
                                <flux:badge :color="$invoice->status === 'paid' ? 'green' : ($invoice->status === 'inactive' ? 'zinc' : 'orange')" variant="pill" size="sm">
                                    {{ $invoice->status === 'paid' ? __('Lunas') : ($invoice->status === 'inactive' ? __('Inactive') : __('Belum Bayar')) }}
                                </flux:badge>
                            </flux:table.cell>
